...
import { LitecoinJsonRpcBaseProvider } from './LitecoinJsonRpcBaseProvider';
import { Block as LitecoinRpcBlock, FeeOptions, MinedTransaction, ProviderOptions } from './types';

export class LitecoinJsonRpcProvider extends Chain<LitecoinJsonRpcBaseProvider> {
    ...
    public async getBlockByHash(blockHash: string, includeTx = false): Promise<Block> {
        let data: LitecoinRpcBlock;

        try {
            data = await this.jsonRpc.send('getblock', [blockHash]); // TODO: This doesn't fit the interface?: https://chainquery.com/litecoin-cli/getblock
        } catch (e) {
        ...
        // TODO: Why transactions need to be retrieved individually? getblock has verbose 2 https://chainquery.com/litecoin-cli/getblock
        if (includeTx) {
    ...
    private async getParsedTransactionByHash(transactionHash: string, addFees = false): Promise<Transaction<LitecoinTransaction>> {
    ...
    private async getTransactionFee(tx: LitecoinTransaction) {
