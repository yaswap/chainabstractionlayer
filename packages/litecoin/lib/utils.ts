...
import coinselect from '@yaswap/litecoinselect';
import coinselectAccumulative from '@yaswap/litecoinselect/accumulative';
...
function decodeRawTransaction(hex: string, network: LitecoinNetwork): LitecoinTransaction {
    const bjsTx = litecoin.Transaction.fromHex(hex);
    ...
    const vin = bjsTx.ins.map((input) => {
        return <Input>{
            ...
            vout: input.index,
            scriptSig: {
                asm: litecoin.script.toASM(input.script),
    ...
    const vout = bjsTx.outs.map((output, n) => {
        ...
        const vout: Output = {
            ...
            n,
            scriptPubKey: {
                asm: litecoin.script.toASM(output.script),
        ...
        try {
            const address = litecoin.address.fromOutputScript(output.script, network);
...
function getPubKeyHash(address: string, network: LitecoinNetwork) {
    const outputScript = litecoin.address.toOutputScript(address, network);
    const type = classify.output(outputScript);
    if (![classify.types.P2PKH, classify.types.P2WPKH].includes(type)) {
        throw new Error(`Litecoin swap doesn't support the address ${address} type of ${type}. Not possible to derive public key hash.`);
    }

    try {
        const bech32 = litecoin.address.fromBech32(address);
        return bech32.data;
    } catch (e) {
        const base58 = litecoin.address.fromBase58Check(address);
        return base58.hash;
    }
}

function validateAddress(_address: AddressType, network: LitecoinNetwork) {
