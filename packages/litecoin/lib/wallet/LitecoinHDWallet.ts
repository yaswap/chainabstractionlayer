...
import { ECPair, ECPairInterface, Psbt, script, Transaction as LitecoinJsTransaction } from 'bitcoinjs-lib';
...
import { AddressType as LitecoinAddressType, LitecoinHDWalletProviderOptions, Input, OutputTarget, PsbtInputTarget } from '../types';
import { LitecoinBaseWalletProvider } from './LitecoinBaseWallet';
import { ILitecoinWallet } from './ILitecoinWallet';

export class LitecoinHDWalletProvider extends LitecoinBaseWalletProvider implements ILitecoinWallet<LitecoinBaseChainProvider> {
    private _mnemonic: string;
    private _seedNode: BIP32Interface;
    private _baseDerivationNode: BIP32Interface;

    constructor(options: LitecoinHDWalletProviderOptions, chainProvider?: Chain<LitecoinBaseChainProvider>) {
        const { mnemonic, baseDerivationPath, addressType = LitecoinAddressType.BECH32, network } = options;
    ...
    public async signMessage(message: string, from: string) {
        ...
        const signature = await signLitecoinMessage(message, keyPair.privateKey, keyPair.compressed);
    ...
    protected async buildTransaction(targets: OutputTarget[], feePerByte?: number, fixedInputs?: Input[]) {
        ...
        const needsWitness = [LitecoinAddressType.BECH32, LitecoinAddressType.P2SH_SEGWIT].includes(this._addressType);
        ...
        for (let i = 0; i < inputs.length; i++) {
            ...
            if (this._addressType === LitecoinAddressType.P2SH_SEGWIT) {
    ...
    public async signBatchP2SHTransaction(
        ...
        for (let i = 0; i < inputs.length; i++) {
            ...
            let sigHash;
            if (segwit) {
                sigHash = tx.hashForWitnessV0(index, inputs[i].outputScript, inputs[i].vout.vSat, LitecoinJsTransaction.SIGHASH_ALL);
            } else {
                sigHash = tx.hashForSignature(index, inputs[i].outputScript, LitecoinJsTransaction.SIGHASH_ALL);
            }

            const sig = script.signature.encode(keyPairs[i].sign(sigHash), LitecoinJsTransaction.SIGHASH_ALL);
            sigs.push(sig);
        }
