...
import { LitecoinNetwork, Input, SwapMode, Transaction as LitecoinTransaction } from '../types';
import {
    calculateFee,
...
import { LitecoinSwapProviderOptions, TransactionMatchesFunction } from './types';

export abstract class LitecoinSwapBaseProvider extends Swap<LitecoinBaseChainProvider, null, ILitecoinWallet<LitecoinBaseChainProvider>> {
    ...
    public findInitiateSwapTransaction(swapParams: SwapParams, blockNumber?: number): Promise<Transaction<any>> {
        this.validateSwapParams(swapParams);
        return this.findSwapTransaction(swapParams, blockNumber, (tx: Transaction<LitecoinTransaction>) =>
    ...
    public async getSwapSecret(claimTxHash: string, initTxHash: string): Promise<string> {
        const claimSwapTransaction: Transaction<LitecoinTransaction> = await this.walletProvider
            .getChainProvider()
    ...
    public async findClaimSwapTransaction(swapParams: SwapParams, initTxHash: string, blockNumber?: number): Promise<Transaction<any>> {
        ...
        const claimSwapTransaction: Transaction<LitecoinTransaction> = await this.findSwapTransaction(
            swapParams,
            blockNumber,
            (tx: Transaction<LitecoinTransaction>) => this.doesTransactionMatchRedeem(initTxHash, tx, false)
        );
    ...
    public async findRefundSwapTransaction(
        ...
        const refundSwapTransaction = await this.findSwapTransaction(swapParams, blockNumber, (tx: Transaction<LitecoinTransaction>) =>
            this.doesTransactionMatchRedeem(initiationTxHash, tx, true)
        );
        return refundSwapTransaction;
    }

    protected onWalletProviderUpdate(_wallet: ILitecoinWallet<LitecoinBaseChainProvider, any>): void {
        // do nothing
    }
     ...
     */
    private async UNSAFE_isSwapRedeemTransaction(transaction: Transaction<LitecoinTransaction>) {
        // eslint-disable-line
        if (transaction._raw.vin.length === 1 && transaction._raw.vout.length === 1) {
            const swapInput = transaction._raw.vin[0];
            const inputScript = this.getInputScript(swapInput);
            const initiationTransaction: Transaction<LitecoinTransaction> = await this.walletProvider
                .getChainProvider()
    ...
    public async updateTransactionFee(tx: Transaction<LitecoinTransaction> | string, newFeePerByte: number) {
        const txHash = typeof tx === 'string' ? tx : tx.hash;
        const transaction: Transaction<LitecoinTransaction> = await this.walletProvider.getChainProvider().getTransactionByHash(txHash);
        if (await this.UNSAFE_isSwapRedeemTransaction(transaction)) {
            const swapInput = transaction._raw.vin[0];
            const inputScript = this.getInputScript(swapInput);
            const initiationTxHash = swapInput.txid;
            const initiationTx: Transaction<LitecoinTransaction> = await this.walletProvider
                .getChainProvider()
    ...
    protected doesTransactionMatchRedeem(initiationTxHash: string, tx: Transaction<LitecoinTransaction>, isRefund: boolean) {
    ...
    protected doesTransactionMatchInitiation(swapParams: SwapParams, transaction: Transaction<LitecoinTransaction>) {
        const swapOutput = this.getSwapOutput(swapParams);
        const swapPaymentVariants = this.getSwapPaymentVariants(swapOutput);
    ...
    ): Promise<Transaction<LitecoinTransaction>>;
}
