...
import {
    ...
    payments,
    TransactionBuilder,
    Transaction as DogecoinJsTransaction,
  } from 'bitcoinjs-lib';
...
export class DogecoinSingleWallet extends Wallet<any, any> implements IDogecoinWallet<DogecoinBaseChainProvider> {
  private _addressType: DogeAddressType;
  private _network: DogecoinNetwork;
  private _ecpair: ECPairInterface;

  constructor(options: DogecoinSingleWalletOptions, chainProvider?: Chain<DogecoinBaseChainProvider>) {
    super(chainProvider);

    this._addressType = options?.addressType || DogeAddressType.LEGACY;
  ...
  public setNetwork(DogecoinNetwork: DogecoinNetwork) {
    this._network = DogecoinNetwork;
  }
  ...
  public async signMessage(message: string) {
    const signature = await signDogecoinMessage(message, this._ecpair.privateKey, this._ecpair.compressed);
