...
import { DogecoinJsonRpcBaseProvider } from './DogecoinJsonRpcBaseProvider';
import { Block as DogecoinRpcBlock, FeeOptions, MinedTransaction, ProviderOptions } from './types';

export class DogecoinJsonRpcProvider extends Chain<DogecoinJsonRpcBaseProvider> {
    ...
    constructor(options: ProviderOptions, feeProvider?: Fee, feeOptions?: FeeOptions) {
        super(options.network, new DogecoinJsonRpcBaseProvider(options), feeProvider);
    ...
    public async getBlockByHash(blockHash: string, includeTx = false): Promise<Block> {
        let data: DogecoinRpcBlock;

        try {
            data = await this.jsonRpc.send('getblock', [blockHash]); // TODO: This doesn't fit the interface?: https://chainquery.com/dogecoin-cli/getblock
        } catch (e) {
        ...
        // TODO: Why transactions need to be retrieved individually? getblock has verbose 2 https://chainquery.com/dogecoin-cli/getblock
        if (includeTx) {
    ...
    private async getParsedTransactionByHash(transactionHash: string, addFees = false): Promise<Transaction<DogecoinTransaction>> {
    ...
    private async getTransactionFee(tx: DogecoinTransaction) {
